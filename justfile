...
# won't catch a real `src/main.rs`. `**/*_tests.rs` skips split-out `#[cfg(test)]` modules
# (e.g. `runtime/render_tests.rs`) — test code is not subject to the CRAP/coverage gates, the
# same as the `tests/**` integration dir (cargo-crap also can't map their coverage and reports
# `null`, which the floor would otherwise flag).
crap_excludes := "--exclude 'examples/**' --exclude 'benches/**' --exclude 'tests/**' --exclude 'xtask/**' --exclude '**/*_tests.rs'"
...
# Run the rediff CLI in the current directory, e.g. `just run diff`, `just run show HEAD`.
[no-cd]
run *args:
    cargo run --quiet --manifest-path {{manifest}} -- {{args}}

# Build and install the rediff binary to ~/.cargo/bin (overwriting any prior version).
install:
    cargo install --path {{justfile_directory()}} --force
...
large-files:
  ...
  found=$(find src tests -type f -name '*.rs' -size +30k)
  if [ -z "$found" ]; then
    echo "(no source files over 30k)"
  else
